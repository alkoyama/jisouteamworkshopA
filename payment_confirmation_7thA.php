        ...
        <?php
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_order'])) {
                try {
                    ...
                    $sql = "INSERT INTO order_management (OID, Date_time, CID, Grand_total_price) VALUES (:oid, :date_time, :cid, :grand_total)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bindParam(':oid', $oid);
                    $stmt->bindParam(':date_time', $currentDateTime);
                    $stmt->bindParam(':cid', $cid);
                    $stmt->bindParam(':grand_total', $grandTotal);  // Grand_totalをバインド
                    $stmt->execute();  // 挿入を実行
